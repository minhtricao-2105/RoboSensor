classdef SpawnObject < handle
    ...
    methods
        ...
        function self = SpanwObject()

        end

        %% RectangularPrism
        % Create a cube in the environment
        function [vertex,face,faceNormals] = RectangularPrism(self, lower,upper,plotOptions,axis_h)
            ...
            vertex(1,:)=lower;
            vertex(2,:)=[upper(1),lower(2), lower(3)];
            vertex(3,:)=[upper(1), upper(2),lower(3)];
            vertex(4,:)=[upper(1),lower(2),upper(3)];
            vertex(5,:)=[lower(1),upper(2), upper(3)];
            vertex(6,:)=[lower(1),lower(2),upper(3)];
